...
Use Render Blueprint to deploy the FastAPI backend from the `render.yaml` at the repo root. Do this **before** deploying the frontend to Vercel.
...
|----------|----------------------|---------------|
| `ALLOWED_ORIGINS` | Yes (after Vercel) | Start with `http://localhost:3000` — update to your Vercel URL later |
...
**Dry-run demo only:** leave `GROQ_API_KEY` and `GEMINI_API_KEY` blank; set `ALLOWED_ORIGINS` to your eventual Vercel URL or `*` is **not supported** — use exact origins comma-separated.

Example for local + Vercel testing:
```
...
After Vercel gives you a URL, update **Render → travel-agent-api → Environment**:
...
Full Vercel steps: [deploy-vercel.md](./deploy-vercel.md)
...
| CORS error from Vercel | Add exact Vercel URL to `ALLOWED_ORIGINS`, redeploy API |
...
Same start command on Railway:
```bash
uvicorn src.api.main:app --host 0.0.0.0 --port $PORT
```
Use the Railway URL as `NEXT_PUBLIC_API_URL` on Vercel.
